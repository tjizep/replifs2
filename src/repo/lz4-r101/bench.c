...
int BMK_benchFile(char **fileNamesTable, int nbFiles, int cLevel) {
    ...
    while (fileIdx < nbFiles) {
        ...
        // Bench
        {
            ...
            for (loopNb = 1; loopNb <= nbIterations; loopNb++) {
                ...
                {
                    size_t i;
                    for (i = 0; i < benchedSize; i++) compressedBuffer[i] = (char) i;
                }     // warmimg up memory
                ...
                while (BMK_GetMilliSpan(milliTime) < TIMELOOP) {
                    for (chunkNb = 0; chunkNb < nbChunks; chunkNb++)
                        chunkP[chunkNb].compressedSize = compP.compressionFunction(chunkP[chunkNb].origBuffer,
                ...
                DISPLAY("%1i-%-14.14s : %9i -> %9i (%5.2f%%),%7.1f MB/s\r", loopNb, inFileName, (int) benchedSize,
                        (int) cSize, ratio, (double) benchedSize / fastestC / 1000.);
                ...
                while (BMK_GetMilliSpan(milliTime) < TIMELOOP) {
                    for (chunkNb = 0; chunkNb < nbChunks; chunkNb++)
                        ...
                        chunkP[chunkNb].compressedSize = LZ4_decompress_fast(chunkP[chunkNb].compressedBuffer,
                ...
                DISPLAY("%1i-%-14.14s : %9i -> %9i (%5.2f%%),%7.1f MB/s ,%7.1f MB/s\r", loopNb, inFileName,
                        (int) benchedSize, (int) cSize, ratio, (double) benchedSize / fastestC / 1000.,
                        (double) benchedSize / fastestD / 1000.);

                // CRC Checking
                crcCheck = XXH32(orig_buff, (unsigned int) benchedSize, 0);
                if (crcOrig != crcCheck) {
                    DISPLAY("\n!!! WARNING !!! %14s : Invalid Checksum : %x != %x\n", inFileName, (unsigned) crcOrig,
                            (unsigned) crcCheck);
                    break;
                }
            }
    ...
    if (nbFiles > 1)
        DISPLAY("%-16.16s :%10llu ->%10llu (%5.2f%%), %6.1f MB/s , %6.1f MB/s\n", "  TOTAL",
                (long long unsigned int) totals, (long long unsigned int) totalz,
                (double) totalz / (double) totals * 100., (double) totals / totalc / 1000.,
