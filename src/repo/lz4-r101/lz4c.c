...
int compress_file(char *input_filename, char *output_filename, int compressionlevel) {
    ...
    out_buff = (char *) malloc(blockSize + CACHELINE);
    headerBuffer = (cunsigned int *) headerBuffer = LITTLE_ENDIAN_32(
    ...
    while (readSize > 0) {
        ...
        } else  // Copy Original Uncompressed
        {
            *(unsigned int *) out_buff = LITTLE_ENDIAN_32(
                    ((unsigned long) readSize) | 0x80000000);   // Add Uncompressed flag
            sizeCheck = fwrite(out_buff, 1, 4, foutput);
            if (sizeCheck != (size_t) (4)) EXM_THROW(34, "Write error : cannot write block header");
    ...
    *(unsigned int *) out_buff = LZ4S_EOS;
                ...
                (double) compressedfilesize / filesize * 100);
    {
        ...
        "Compressed %llu bytes into %llu bytes ==> %.2f%%\n",
                (unsigned long long) filesize, (unsigned long long) compressedfilesize, (double) compressedfilesize /
                                                                                        filesize * 100);
        {
            double seconds = (double) (end - start) / CLOCKS_PER_SEC;
            if (verbose) DISPLAY("Done in %.2f s ==> %.2f MB/s\n", seconds, (double) filesize / seconds / 1024 / 1024);
    ...
    unsigned long long decodeLegacyStream(FILE *finput, FILE *foutput) {
        ...
        while (1) {
            ...
            if (blockSize > LZ4_COMPRESSBOUND(LEGACY_BLOCKSIZE)) {   // Cannot read next block : maybe new stream ?
    ...
    unsigned long long selectDecoder(FILE *finput, FILE *foutput) {
        ...
        switch (magicNumber) {
            ...
            default:
                if (ftell(finput) == MAGICNUMBER_SIZE) EXM_THROW(44,
                                                                 "Unrecognized header : file cannot be decoded");   // Wrong magic number at the beginning of 1st stream
    ...
    int main(int argc, char **argv) {
        ...
        // Select behavior
        programName = argv[0];
        if (strstr(programName, UNLZ4)) {
            decode = 1;
            silence = 1;
        }
        else if (strstr(programName, LZ4CAT)) {
            decode = 1;
            silence = 1;
        ...
        if (!output_filename) {
            ...
            } else                // decompression (input file must respect format extension ".lz4")
            {
                ...
                if (outl != inl - 5) {
                    DISPLAY("Cannot automatically decide an output filename\n");
                    badusage();
                }
            }
        }
