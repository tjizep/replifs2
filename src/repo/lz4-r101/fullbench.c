...
# define MAXCOMPRESSIONCHAR '3'
    static char *compressionNames[] = {"LZ4_compress", "LZ4_compress_limitedOutput", "LZ4_compressHC",
...
# define MAXDECOMPRESSIONCHAR '4'
    static char *decompressionNames[] = {"LZ4_decompress_fast", "LZ4_decompress_fast_withPrefix64k",
        ...
        // Bench
        {
            ...
            for (cAlgNb = 0; (cAlgNb < NB_COMPRESSION_ALGORITHMS) && (compressionTest); cAlgNb++) {
                ...
                for (loopNb = 1; loopNb <= nbIterations; loopNb++) {
                    ...
                    cSize = 0;
                    for (chunkNb = 0; chunkNb < nbChunks; chunkNb++) cSize += chunkP[chunkNb].compressedSize;
                    ratio = (double) cSize / (double) benchedSize * 100.;
                    DISPLAY("%1i-%-19.19s : %9i -> %9i (%5.2f%%),%7.1f MB/s\r", loopNb, cName, (int) benchedSize,
                            (int) cSize, ratio, (double) benchedSize / bestTime / 1000.);
                }

                if (ratio < 100.)
                    DISPLAY("%-21.21s : %9i -> %9i (%5.2f%%),%7.1f MB/s\n", cName, (int) benchedSize, (int) cSize,
                            ratio, (double) benchedSize / bestTime / 1000.);
                else
            ...
            for (chunkNb = 0; chunkNb < nbChunks; chunkNb++) {
                chunkP[chunkNb].compressedSize = LZ4_compress(chunkP[chunkNb].origBuffer,
                                                              chunkP[chunkNb].compressedBuffer,
                                                              chunkP[chunkNb].origSize);
            ...
            for (dAlgNb = 0; (dAlgNb < NB_DECOMPRESSION_ALGORITHMS) && (decompressionTest); dAlgNb++) {
                ...
                for (loopNb = 1; loopNb <= nbIterations; loopNb++) {
                    ...
                    while (BMK_GetMilliSpan(milliTime) < TIMELOOP) {
                        for (chunkNb = 0; chunkNb < nbChunks; chunkNb++) {
                            int decodedSize = decompressionFunction(chunkP[chunkNb].compressedBuffer,
                                                                    chunkP[chunkNb].origBuffer,
                                                                    chunkP[chunkNb].compressedSize,
                                                                    chunkP[chunkNb].origSize);
                            if (chunkP[chunkNb].origSize != decodedSize)
                                DISPLAY("ERROR ! %s() == %i != %i !! \n", dName, decodedSize,
                                        chunkP[chunkNb].origSize), exit(1);
                        }
                        nb_loops++;
                    }
                    milliTime = BMK_GetMilliSpan(milliTime);
                    ...
                    DISPLAY("%1i-%-24.24s :%10i -> %7.1f MB/s\r", loopNb, dName, (int) benchedSize,
    ...
    if (nbFiles > 1) {
        ...
        for (AlgNb = 0; (AlgNb < NB_COMPRESSION_ALGORITHMS) && (compressionTest); AlgNb++) {
            ...
            DISPLAY("%-21.21s :%10llu ->%10llu (%5.2f%%), %6.1f MB/s\n", cName, (long long unsigned int) totals,
                    (long long unsigned int) totalCSize[AlgNb], (double) totalCSize[AlgNb] / (double) totals * 100.,
        ...
        for (AlgNb = 0; (AlgNb < NB_DECOMPRESSION_ALGORITHMS) && (decompressionTest); AlgNb++) {
            ...
            DISPLAY("%-21.21s :%10llu -> %6.1f MB/s\n", dName, (long long unsigned int) totals,
