...
#define D_OFF(x) x->d_seekoff
#else
#define D_OFF(x) x->d_off
#endif
...
static void sfs_getattr(fuse_req_t req, fuse_ino_t ino, fuse_file_info *fi) {
    ...
    if (fcntl(inode.fd, F_GETPATH, filePath) == -1) {
        cerr << "fcntl error " << strerror(errno) << endl;
        fuse_reply_err(req, errno);
        return;
    }
    auto res = fstatat(inode.fd, filePath, &attr,
                       AT_EMPTY_PATH | AT_SYMLINK_NOFOLLOW);
    if (res == -1) {
        cerr << "fstatat error " << strerror(errno) << endl;
        fuse_reply_err(req, errno);
        return;
    }
...
static int mknodat(int dirfd, const char *filename, mode_t mode, dev_t dev) {
...
static void sfs_forget(fuse_req_t req, fuse_ino_t ino, unsigned long nlookup) {
...
static void do_readdir(fuse_req_t req, fuse_ino_t ino, size_t size,
    ...
    while (1) {
        ...
        off_t current = telldir(d->dp);
        d->offset = current; //D_OFF(entry);
        ...
        if (plus) {
            ...
            entsize = fuse_add_direntry_plus(req, p, rem, entry->d_name, &e, current);
...
#else
            cerr << "Platform does not support this operation " << endl;
#endif
        } else {
            e.attr.st_ino = entry->d_ino;
            e.attr.st_mode = entry->d_type; // << 12;
            entsize = fuse_add_direntry(req, p, rem, entry->d_name, &e.attr, current);
        ...
        if (fs.debug) {
            cerr << "DEBUG: readdir(): added to buffer: " << entry->d_name
                 << ", ino " << e.attr.st_ino << ", offset " << current << endl;
        }
    }
    err = 0;
    error:
    ...
    if (err && rem == size) {
        ...
        int en;
        if ((en = fuse_reply_buf(req, buf, size - rem)) != 0)
            cerr << "reply buf failed " << strerror(-en) << endl;
    }
    delete[] buf;
    return;
}
...
int main(int argc, char *argv[]) {
    ...
    fuse_lowlevel_ops sfs_oper{0};
    assign_operations(sfs_oper);

    //struct fuse_args args = FUSE_ARGS_INIT(argc, argv);
    char *mountpoint = nullptr;
    int err = -1;
    fuse_chan *ch = nullptr;

    if (fuse_parse_cmdline(&args, &mountpoint, NULL, NULL) != -1 &&
        (ch = fuse_mount(argv[2], &args)) != NULL) {
        struct fuse_session *se;

        se = fuse_lowlevel_new(&args, &sfs_oper, sizeof(sfs_oper), &fs);
        if (se != nullptr) {
